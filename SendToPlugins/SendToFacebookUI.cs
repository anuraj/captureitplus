            ...
            using (var commonAuthDlg = new CommonAuthDialog(AuthUrl, AppCallbackUrl, AppId))
            {
                if (commonAuthDlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    webClient.UploadFileCompleted += (o, ev) =>
                    {
                        ...
                        if (ev.Error == null)
                        {
                            MessageBox.Show("Image successfully uploaded.", "Send to Facebook", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show(string.Format("Unable to upload image to facebook{0}Details : {1}", Environment.NewLine, ev.Error.Message),
                                "Send to Facebook", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    };
